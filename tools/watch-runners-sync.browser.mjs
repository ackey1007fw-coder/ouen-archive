...
try{for(const width of [390,430,1280]){
  ...
  try{
    ...
    await check('Mixch soft navigation into a live room mounts the correct panel',async()=>{await soft('/u/100001/live',live);await part('title').waitFor();assert.match(await part('title').innerText(),/v0\.3\.0/);assert.equal(await part('watchControls').isVisible(),true);});
