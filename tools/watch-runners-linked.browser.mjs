...
try{for(const width of [390,430,1280]){
 ...
 try{
  ...
  await check('ad entry stores and pauses the stream countdown',async()=>{await part('openAds').click();await page.waitForURL('**/lottery/ad_reward');await panel().waitFor();assert.match(await panel().innerText(),/保存して停止中/);assert.equal(store.get('srmr_progress_v3_ad_return').slug,'test-one');});
  await tick(40000);await panel().getByText('配信へ戻る',{exact:true}).click();await page.waitForURL('**/lite/test-one');await panel().waitFor();
  await check('return from ads does not count ad time and needs explicit resume',async()=>{await page.evaluate(()=>{window.fixturePaused=false;});const before=Number(await part('time').innerText());await tick(5000);assert.equal(Number(await part('time').innerText()),before);assert.ok(before<=remaining&&before>=remaining-2);assert.match(await part('status').innerText(),/再開/);await part('pause').click();await tick(4000);assert.ok(Number(await part('time').innerText())<before);});
  await check('ad watch pages have no helper overlay',async()=>{await page.goto('https://www.showroom-live.com/lottery/ad_reward/1/watch');await tick();assert.equal(await panel().count(),0);});
