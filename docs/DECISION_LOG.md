...
## 2026-08-20 — OGPシェア画像は4人のポートレート＋大きなタイトルにする

**Decision**

`src/app/opengraph-image.tsx` を、テキストのみの構成から「上段に4人のポートレート、下段に濃色バンドで `応援アーカイブ`」の構成へ差し替える。

掲載順は固定人物順のまま `mily → yukako → riri → chizuru`。MAKOはこのシェア画像には掲載しない。Milyのタイルだけ他より広い（360px / 290 / 280 / 270）。

**Reason**

シェア時の第一印象を強くするため。テキストのみのOGPはタイムライン上で情報が伝わらず、クリック理由にならなかった。顔が出ることでポータルの中身が一目で伝わる。

MAKO非掲載とMilyタイルの拡大は、いずれもオーナーの明示指示である。

**Relationship to 2026-08-18 「5人は固定順・同等weight」**

その判断は**ポータル本体の人物カード**に対するものであり、今も有効。人物カードは5人すべてを同等weight・固定順で描画し続ける。今回の例外は**シェア画像だけ**に閉じている。

**Do not**

- 人物カードUIへこの例外を波及させない（5人・同等weightのまま）
- 掲載順を人気・engagementで並べ替えない
- 顔の生成・合成・レタッチをしない（既存ポートレートのトリミングのみ）

**Implementation notes**

- 素材は `public/portraits/` の既存ポートレートのみ。顔位置に合わせてトリミングし、1.5倍解像度で `assets/og-portraits/*.jpg` に書き出したもの（合計約148KB）。元ファイルは変更しない。
...
## 2026-09-11 — 視聴件数と取得済み配信の除外履歴は別に保持する

**Decision**
SHOWROOMの03:00 / 15:00で切り替えるのは端末の件数。取得済みとして記録した配信の除外履歴は周回再開・時間帯切替・件数リセットで消さない。候補の開始日時が最後の記録時刻より後なら新しい配信として扱い、日時不明の場合は安全側で除外する。

**Reason**
オーナーが、一度取得した配信へ再入室する問題を報告し、再び候補にしないよう依頼。件数のリセットだけで履歴まで消すと継続配信へ無駄に戻ってしまうため。

**Do not**
公式の獲得履歴を自動取得したと表現しない。同じルームの新しい配信まで永久に除外しない。履歴を消した旧版の情報を推測で復元しない。半自動だから規約適合だと断定しない。

**Related**
PR #14、`docs/WATCH-RUNNERS.md`。この判断ログは当該PRのmerge承認を意味しない。


## 2026-09-11 — 公式進捗は任意の読取表示とし、配信別履歴に変換しない

**Decision**
オーナーの公式情報読取の依頼に対し、同一オリジンの確認済み進捗GETだけを初期OFFの任意機能として許可。直接認証情報を扱わず、報酬受取・操作系API・他ホストへの転送を行わない。読めたタイトルと進捗値は画面内に一時表示し、端末件数・除外履歴を自動で変更しない。

**Reason**
ミッションの現在値・目標値は、過去のどの配信で獲得したかという全履歴とは異なる。未照合のレスポンス形式や単位を推測すると、未取得の配信を誤除外したり件数を二重計上したりするため。未知・失敗・前枠の応答は不明扱いとする。

**Scope**
PR #14のSR v1.3.0に限定。ミクチャは追加通信なし。今回の追記はmerge承認や規約適合の保証を意味しない。


## 2026-09-12 — 明示ONの公式回数連動と、広告時間の分離

**Decision**
公式進捗への連動を求める追加依頼を受け、正しい回数として検証できる値だけを残り回数・開始停止へ利用する。SRは公式MissionCardの受取算式、ミクチャは新しい公式成功通知のN/Mを利用。最小数値と選択設定は同一タブのsessionStorageに保存し、未知・期限切れを0として扱わない。端末の配信別履歴は書き換えない。

**Reason / Limits**
表示だけのv1.3から、残り回数判断にも反映するため。合計値は配信別獲得履歴ではない点を維持。広告では配信計測を停止し、広告の映像を視聴ミッションへ二重計上しない。2026-09-11の保存制限はこの明示ONの最小数値保存だけを拡張し、全文応答・アカウント情報・外部送信を引き続き禁止。merge承認や規約適合保証にはしない。


## 2026-09-12 — 広告は別タブ、配信の背景時間は数えない

**Decision**
SHOWROOM広告は配信ページから別タブで開き、元の配信タブを保持する。広告タブを見ている間の非表示時間は配信視聴秒数へ加算せず、元の配信タブへ戻った時にメディア進行を確認して自動再開する。手動の再開操作は不要とする。

**Reason / Limits**
iPhone Safariで別タブ中の元タブを実視聴中とは扱えず、OSがメディアを停止する場合もあるため。背景時間まで30秒視聴に加えると実視聴と端末記録が乖離する。広告自体の再生・抽選・報酬受取は公式UIのまま。別タブ化は操作負担の軽減だけを目的とし、同時計上の保証や自動化にはしない。
