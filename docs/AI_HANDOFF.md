...
- PR #14 / `feat/watch-runners-progress-20260911` はSR v1.4.1とミクチャ視聴メモv0.3.1β。未merge、オーナーの明示承認待ち。現在の状態はPRを再取得すること。
...
- 広告ボタンは配信計測を保存・停止して公式へ。戻って手動再開するまで進めない。広告視聴との同時計上・自動再生・抽選・受取は未実装。
